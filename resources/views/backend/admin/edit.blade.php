...
<form class="form-horizontal" role="form" method="POST" action="{{ route('backend.admins.update', $admin->admin_id) }}" onSubmit="return $.checkForm();" autocomplete="off"> {{ csrf_field() }}
	...
	<div class="form-group{{ $errors->has('admin_id') ? ' has-error' : '' }}">
		<label class="col-sm-6 col-lg-2 control-label"><span class=" text-danger"><b>˚</b></span>Admin Id</label>
		<div class="col-sm-8 col-lg-3">
			<input class="form-control" id="admin_id" name="admin_id" type="text" value="{{ old('admin_id', $admin->admin_id) }}" disabled>
			{!! ($errors->first('admin_id')) ? showErrorBlock($errors->first('admin_id')) : '' !!} 
		</div>
	</div>
	...
	<div class="form-group{{ $errors->has('name') ? ' has-error' : '' }}">
		...
		<div class="col-sm-4 col-lg-3">
			<input class="form-control" id="username" name="username" type="text" value="{{ old('username', $admin->username) }}">
		</div>
	</div>
	...
	<div class="form-group{{ $errors->has('email') ? ' has-error' : '' }}">
		<label class="col-sm-4 col-lg-2 control-label"><span class=" text-danger"><b>˚</b></span>Email</label> 
			<div class="col-sm-8 col-lg-3">
				<input class="form-control" id="email" name="email" type="text" value="{{ old('email', $admin->email) }}">
			</div>
	</div>
	...
	<div class="form-group{{ $errors->has('status') ? ' has-error' : '' }}">
		<label class="col-sm-4 col-lg-2 control-label">Status</label>
			<div class="col-sm-8 col-lg-10">
				<input class="bootstrap-switch statusCheckbox" id="status" name="status" type="checkbox" value="{{ old('status', $admin->status) }}">
			</div>
	</div>
